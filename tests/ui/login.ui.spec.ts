...
test("successful login", async ({ page }) => {
  ...
  await expect(
    page.getByRole("heading", { name: "Hi Slawomir!" })
  ).toBeVisible();
  await expect(
    page.getByText("You're logged in! Congratulations :)")
  ).toBeVisible();

  // Verify the presence of expected elements after login
  await expect(
    page.getByRole("heading", { name: "All registered users:" })
  ).toBeVisible();
  await expect(page.getByRole("link", { name: "Logout" })).toBeVisible();
  await expect(
    page.getByRole("link", { name: "Add more users" })
  ).toBeVisible();
